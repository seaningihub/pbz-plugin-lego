...
class SLGATimelineOutliner : public STreeView<TSharedRef<FLGATimelineTrack>>
{
public:
	SLATE_BEGIN_ARGS(SLGATimelineOutliner) {}
	...
	~SLGATimelineOutliner();

	void Construct(const FArguments& InArgs, const TSharedRef<FLGATimelineModelBase>& InAnimModel, const TSharedRef<SLGATimelineTrackArea>& InTrackArea);
	...
	/** STreeView interface */
	virtual void Private_SetItemSelection( TSharedRef<FLGATimelineTrack> TheItem, bool bShouldBeSelected, bool bWasUserDirected = false ) override;
	virtual void Private_ClearSelection() override;
	virtual void Private_SelectRangeFromCurrentTo( TSharedRef<FLGATimelineTrack> InRangeSelectionEnd ) override;

	/** Structure used to cache physical geometry for a particular track */
	struct FCachedGeometry
	{
		FCachedGeometry(TSharedRef<FLGATimelineTrack> InTrack, float InTop, float InHeight)
			: Track(MoveTemp(InTrack))
			, Top(InTop)
			, Height(InHeight)
		{}

		TSharedRef<FLGATimelineTrack> Track;
		float Top;
		float Height;
	};

	/** Get the cached geometry for the specified track */
	TOptional<FCachedGeometry> GetCachedGeometryForTrack(const TSharedRef<FLGATimelineTrack>& InTrack) const;

	/** Compute the position of the specified track using its cached geometry */
	TOptional<float> ComputeTrackPosition(const TSharedRef<FLGATimelineTrack>& InTrack) const;

	/** Report geometry for a child row */
	void ReportChildRowGeometry(const TSharedRef<FLGATimelineTrack>& InTrack, const FGeometry& InGeometry);

	/** Called when a child row widget has been added/removed */
	void OnChildRowRemoved(const TSharedRef<FLGATimelineTrack>& InTrack);
...
private:
	...
	TSharedRef<ITableRow> HandleGenerateRow(TSharedRef<FLGATimelineTrack> Item, const TSharedRef<STableViewBase>& OwnerTable);

	/** Get the children of an outliner item */
	void HandleGetChildren(TSharedRef<FLGATimelineTrack> Item, TArray<TSharedRef<FLGATimelineTrack>>& OutChildren);

	/** Record the expansion state when it changes */
	void HandleExpansionChanged(TSharedRef<FLGATimelineTrack> InItem, bool bIsExpanded);
...
private:
	...
	TWeakPtr<FLGATimelineModelBase> AnimModel;

	/** The track area */
	TSharedPtr<SLGATimelineTrackArea> TrackArea;
	...
	TMap<TSharedRef<FLGATimelineTrack>, FCachedGeometry> CachedTrackGeometry;
