...
void SLGATimelineTimingTrack::Construct(const FArguments& Args)
{
	SLGATrack::FArguments BaseArgs;
	...
	SLGATrack::Construct(BaseArgs);
}

void SLGATimelineTimingTrack::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	struct NodeData
	{
		NodeData(TSharedRef<SLGATrackNode>& NodeRef, const FGeometry& Geometry)
			: Node(NodeRef)
		{
		...
		TSharedRef<SLGATrackNode> Node;	// The node widget
	...
	for (int32 TrackIndex = 0; TrackIndex < NumNodes; ++TrackIndex)
	{
		TSharedRef<SLGATrackNode> TrackNode = (SortedNodeData[TrackIndex].Node);
