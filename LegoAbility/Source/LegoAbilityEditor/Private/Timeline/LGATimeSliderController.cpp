...
struct FLGATimeSliderController::FScrubRangeToScreen
{
	double ViewStart;
...
FLGATimeSliderController::FLGATimeSliderController(const FTimeSliderArgs& InArgs, TWeakPtr<FLGATimelineModelBase> InWeakModel, TWeakPtr<SLGATimeline> InWeakTimeline, TSharedPtr<INumericTypeInterface<double>> InSecondaryNumericTypeInterface)
...
FFrameTime FLGATimeSliderController::ComputeFrameTimeFromMouse(const FGeometry& Geometry, FVector2D ScreenSpacePosition, FScrubRangeToScreen RangeToScreen, bool CheckSnapping) const
{
...
FLGATimeSliderController::FScrubPixelRange FLGATimeSliderController::GetHitTestScrubberPixelRange(FFrameTime ScrubTime, const FScrubRangeToScreen& RangeToScreen) const
{
...
FLGATimeSliderController::FScrubPixelRange FLGATimeSliderController::GetScrubberPixelRange(FFrameTime ScrubTime, const FScrubRangeToScreen& RangeToScreen) const
{
	return GetScrubberPixelRange(ScrubTime, GetTickResolution(), GetDisplayRate(), RangeToScreen);
}

FLGATimeSliderController::FScrubPixelRange FLGATimeSliderController::GetScrubberPixelRange(FFrameTime ScrubTime, FFrameRate Resolution, FFrameRate PlayRate, const FScrubRangeToScreen& RangeToScreen, float DilationPixels) const
{
...
struct FLGATimeSliderController::FDrawTickArgs
{
...
void FLGATimeSliderController::DrawTicks(FSlateWindowElementList& OutDrawElements, const TRange<double>& ViewRange, const FScrubRangeToScreen& RangeToScreen, FDrawTickArgs& InArgs) const
{
	TSharedPtr<SLGATimeline> Timeline = WeakTimeline.Pin();
	if (!Timeline.IsValid())
	{
		return;
	}
...
int32 FLGATimeSliderController::OnPaintTimeSlider(bool bMirrorLabels, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
int32 FLGATimeSliderController::DrawSelectionRange(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args) const
{
...
int32 FLGATimeSliderController::DrawPlaybackRange(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args) const
{
...
int32 FLGATimeSliderController::DrawEditableTimes(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FScrubRangeToScreen& RangeToScreen) const
{
...
FReply FLGATimeSliderController::OnMouseButtonDown(SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	DistanceDragged = 0;
...
FReply FLGATimeSliderController::OnMouseButtonUp(SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	else if (bHandleLeftMouseButton)
	{
		...
		else if (MouseDragType == DRAG_TIME)
		{
			...
			if (!MouseEvent.IsControlDown())
			{
				...
				WeakModel.Pin()->Snap(Time, SnapMargin, false);
			}
...
FReply FLGATimeSliderController::OnMouseMove(SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void FLGATimeSliderController::CommitScrubPosition(FFrameTime NewValue, bool bIsScrubbing)
{
...
FReply FLGATimeSliderController::OnMouseWheel(SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FCursorReply FLGATimeSliderController::OnCursorQuery(TSharedRef<const SWidget> WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
...
int32 FLGATimeSliderController::OnPaintViewArea(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, bool bEnabled, const FPaintViewAreaArgs& Args) const
{
	const ESlateDrawEffect DrawEffects = bEnabled ? ESlateDrawEffect::None : ESlateDrawEffect::DisabledEffect;
	...
	TSharedPtr<FLGATimelineModelBase> TimelineModel = WeakModel.Pin();
	if (TimelineModel.IsValid())
	{
		const FLinearColor LineColor = GetDefault<UPersonaOptions>()->SectionTimingNodeColor;

		// Draw all the times that we can drag in the timeline
		for (double Time : TimelineModel->GetEditableTimes())
		{
			const float LinePos = RangeToScreen.InputToLocalX(Time);
...
FFrameTime FLGATimeSliderController::GetFrameTimeFromMouse(const FGeometry& Geometry, FVector2D ScreenSpacePosition) const
{
...
TSharedRef<SWidget> FLGATimeSliderController::OpenSetPlaybackRangeMenu(FFrameNumber FrameNumber)
{
	FMenuBuilder MenuBuilder(true, nullptr);
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
	FText CurrentTimeText = FText::FromString(TimeSliderArgs.NumericTypeInterface->ToString(FrameNumber.Value));
	MenuBuilder.BeginSection("AnimSelectionRangeMenu", FText::Format(LOCTEXT("SelectionRangeTextFormat", "Selection Range ({0}):"), CurrentTimeText));
	{
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(
				...
				FCanExecuteAction::CreateLambda([&]{ return SelectionRange.IsEmpty() || FrameNumber < DiscreteExclusiveUpper(SelectionRange); })
			)
		);
		...
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(
				...
				FCanExecuteAction::CreateLambda([&]{ return SelectionRange.IsEmpty() || FrameNumber >= DiscreteInclusiveLower(SelectionRange); })
			)
		);
...
void FLGATimeSliderController::ClampViewRange(double& NewRangeMin, double& NewRangeMax)
{
...
void FLGATimeSliderController::SetViewRange(double NewRangeMin, double NewRangeMax, EViewRangeInterpolation Interpolation)
{
...
void FLGATimeSliderController::SetClampRange(double NewRangeMin, double NewRangeMax)
{
...
void FLGATimeSliderController::SetPlayRange(FFrameNumber RangeStart, int32 RangeDuration)
{
...
TRange<FFrameNumber> FLGATimeSliderController::GetTimeBounds() const
{
	return TimeSliderArgs.PlaybackRange.Get(TRange<FFrameNumber>());
}

bool FLGATimeSliderController::ZoomByDelta(float InDelta, float MousePositionFraction)
{
...
void FLGATimeSliderController::PanByDelta(float InDelta)
{
	TRange<double> LocalViewRange = TimeSliderArgs.ViewRange.Get().GetAnimationTarget();
...
bool FLGATimeSliderController::HitTestRangeStart(const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel) const
{
...
bool FLGATimeSliderController::HitTestRangeEnd(const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel) const
{
...
bool FLGATimeSliderController::HitTestTime(const FScrubRangeToScreen& RangeToScreen, double Time, float HitPixel) const
{
...
void FLGATimeSliderController::SetPlaybackRangeStart(FFrameNumber NewStart)
{
	TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();

	if (NewStart <= DiscreteExclusiveUpper(PlaybackRange))
	{
		TimeSliderArgs.OnPlaybackRangeChanged.ExecuteIfBound(TRange<FFrameNumber>(NewStart, PlaybackRange.GetUpperBound()));
	}
}

void FLGATimeSliderController::SetPlaybackRangeEnd(FFrameNumber NewEnd)
{
	TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();

	if (NewEnd >= DiscreteInclusiveLower(PlaybackRange))
	{
		TimeSliderArgs.OnPlaybackRangeChanged.ExecuteIfBound(TRange<FFrameNumber>(PlaybackRange.GetLowerBound(), NewEnd));
	}
}

void FLGATimeSliderController::SetSelectionRangeStart(FFrameNumber NewStart)
{
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
	...
	else if (NewStart <= DiscreteExclusiveUpper(SelectionRange))
	{
		TimeSliderArgs.OnSelectionRangeChanged.ExecuteIfBound(TRange<FFrameNumber>(NewStart, SelectionRange.GetUpperBound()));
	}
}

void FLGATimeSliderController::SetSelectionRangeEnd(FFrameNumber NewEnd)
{
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
	...
	else if (NewEnd >= DiscreteInclusiveLower(SelectionRange))
	{
		TimeSliderArgs.OnSelectionRangeChanged.ExecuteIfBound(TRange<FFrameNumber>(SelectionRange.GetLowerBound(), NewEnd));
	}
}

int32 FLGATimeSliderController::HitTestTimes(const FScrubRangeToScreen& RangeToScreen, float HitPixel) const
{
...
void FLGATimeSliderController::SetEditableTime(int32 TimeIndex, float Time, bool bIsDragging)
{
	TSharedPtr<FLGATimelineModelBase> Model = WeakModel.Pin();
