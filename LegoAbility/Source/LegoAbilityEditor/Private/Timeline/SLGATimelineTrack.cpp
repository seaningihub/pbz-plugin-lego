...
void SLGATimelineTrack::Construct(const FArguments& InArgs, const TSharedRef<FLGATimelineTrack>& InTrack, const TSharedRef<SLGATimelineOutliner>& InOutliner)
{
	WeakTrack = InTrack;
...
int32 SLGATimelineTrack::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	static const FName BorderName("AnimTimeline.Outliner.DefaultBorder");
	static const FName SelectionColorName("SelectionColor");

	TSharedPtr<FLGATimelineTrack> Track = WeakTrack.Pin();
	if (Track.IsValid())
	{
		if (Track->IsVisible())
		{
...
void SLGATimelineTrack::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	FVector2D ThisFrameDesiredSize = GetDesiredSize();

	if (LastDesiredSize.IsSet() && ThisFrameDesiredSize.Y != LastDesiredSize.GetValue().Y)
	{
		TSharedPtr<SLGATimelineOutliner> Outliner = WeakOutliner.Pin();
		if (Outliner.IsValid())
		{
...
FVector2D SLGATimelineTrack::ComputeDesiredSize(float LayoutScale) const
{
	TSharedPtr<FLGATimelineTrack> Track = WeakTrack.Pin();
	if (Track.IsValid())
	{
...
float SLGATimelineTrack::GetPhysicalPosition() const
{
	TSharedPtr<FLGATimelineTrack> Track = WeakTrack.Pin();
	if (Track.IsValid())
	{
...
void SLGATimelineTrack::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SLGATimelineTrack::OnMouseLeave(const FPointerEvent& MouseEvent)
{
...
FReply SLGATimelineTrack::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	TSharedPtr<FLGATimelineTrack> Track = WeakTrack.Pin();
	TSharedPtr<SLGATimelineOutliner> Outliner = WeakOutliner.Pin();
