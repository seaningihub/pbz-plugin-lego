...
class SLGATimelineOutlinerItem;
class SWidget;
...
class FLGATimelineTrack : public TSharedFromThis<FLGATimelineTrack>
{
public:
	static const float OutlinerRightPadding;

	FLGATimelineTrack(const FText& InDisplayName, const FText& InToolTipText, const TSharedPtr<FLGATimelineModelBase>& InModel, bool bInIsHeaderTrack = false)
		: Model(InModel)
	...
	virtual ~FLGATimelineTrack() {}

	/** Get the children of this object */
	const TArray<TSharedRef<FLGATimelineTrack>>& GetChildren() { return Children; }

	/** Add a child track */
	void AddChild(const TSharedRef<FLGATimelineTrack>& InChild) { Children.Add(InChild); }
	...
	TSharedRef<FLGATimelineModelBase> GetModel() const { return Model.Pin().ToSharedRef(); }
	 ...
	 */
	bool Traverse_ChildFirst(const TFunctionRef<bool(FLGATimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);
	 ...
	 * @return  true where the client prematurely exited the iteration, false otherwise
	 */
	bool Traverse_ParentFirst(const TFunctionRef<bool(FLGATimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);
	 ...
	 * @return  true where the client prematurely exited the iteration, false otherwise
	 */
	bool TraverseVisible_ChildFirst(const TFunctionRef<bool(FLGATimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);
	 ...
	 * @return  true where the client prematurely exited the iteration, false otherwise
	 */
	bool TraverseVisible_ParentFirst(const TFunctionRef<bool(FLGATimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);

	/** Generate a widget for the outliner for this ability timeline (locates on the left side of timeline panel) */
	virtual TSharedRef<SWidget> GenerateContainerWidgetForOutliner(const TSharedRef<SLGATimelineOutlinerItem>& InRow);

	/** Generate a widget for the track area for this ability timeline (locates on the right side of timeline panel) */
...
protected:
	...
	TSharedRef<SWidget> GenerateStandardOutlinerWidget(const TSharedRef<SLGATimelineOutlinerItem>& InRow, bool bWithLabelText, TSharedPtr<SBorder>& OutOuterBorder, TSharedPtr<SHorizontalBox>& OutInnerHorizontalBox);
...
protected:
	/** Child objects */
	TArray<TSharedRef<FLGATimelineTrack>> Children;
