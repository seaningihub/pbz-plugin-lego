...
class SLGATimelineTransportControls;

/**
 * Implements the anim timeline widget.
 */
class SLGATimeline : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SLGATimeline) {}
...
private:
	...
	FReply ResizeTimline();

private:
	/** Anim timeline model */
	TWeakPtr<FLGATimelineModel> Model;

	/** Outliner widget */
	TSharedPtr<SLGATimelineOutliner> Outliner;

	/** Track area widget */
	TSharedPtr<SLGATimelineTrackArea> TrackArea;

	/** The time slider controller */
	TSharedPtr<FLGATimeSliderController> TimeSliderController;
